"""Get config dictionary from config file, if present, else from defaults."""
...
def test_get_config_from_default_config_file_if_present(tmp_path):
    ...
    assert get_config(configfile=DEFAULT_CONFIGFILE_NAME, defaults_yaml=YAML_CONFIGFILE_CONTENTS)["prefixes"] == {
        ":": "http://example.org/",
        "dcterms:": "http://purl.org/dc/terms/",
    }
    assert get_config(configfile=DEFAULT_CONFIGFILE_NAME, defaults_yaml=YAML_CONFIGFILE_CONTENTS) == {
        "default_shape_name": ":default",
...
def test_exit_if_configfile_has_bad_yaml(tmp_path):
    ...
    with pytest.raises(ConfigError):
        get_config(configfile=DEFAULT_CONFIGFILE_NAME, defaults_yaml=YAML_CONFIGFILE_CONTENTS)
