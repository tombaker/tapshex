...
def test_tapshex_config_from_builtins(tmp_path):
...
def test_tapshex_config_from_passed_nondefault_yaml():
    ...
    """
    config_dict = tapshex_config(nondefault_configyaml_str=some_configyaml)
    assert config_dict.get("prefixes")
    assert config_dict.get("default_shape_identifier")
    assert ":" in config_dict.get("prefixes")

def test_assigned_default_if_tapshex_config_passed_yaml_with_empty_shapeid(tmp_path):
...
def test_tapshex_config_from_default_configfile(tmp_path):
    """Get config dict from default config file DEFAULT_CONFIGFILE if present."""
    os.chdir(tmp_path)
    nondefault_yaml = """
    default_shape_identifier: "default"
    prefixes:
    ...
    - literal
    """
    Path(DEFAULT_CONFIGFILE).write_text(nondefault_yaml)
    assert Path(DEFAULT_CONFIGFILE).is_file()
    config_dict = tapshex_config()
...
def test_tapshex_config_from_nondefault_configfile_if_specified(tmp_path):
    """Get config dict from nondefault config file DEFAULT_CONFIGFILE if specified."""
    os.chdir(tmp_path)
    ...
    config_dict = tapshex_config(nondefault_configfile_name=nondefault_configfile_name)
    assert config_dict["extra_element_aliases"]["ShapID"] == "shapeID"
    # Correctly adds colons to prefixes if necessary.
    assert "school:" in config_dict.get("prefixes")
    assert ":" in config_dict.get("prefixes")

def test_tapshex_config_from_passed_nondefault_yaml_even_if_prefixes_lack_colons():
    """Prefixes without colons are tolerated in YAML docs but added by tapshex_config."""
    some_configyaml = """\
    default_shape_identifier: "default"
    prefixes:
        "": "http://example.org/"
...
def test_extra_shape_elements(tmp_path):
    ...
    os.chdir(tmp_path)
    config_dict = tapshex_config()
    expected_shems = ['shapeID', 'shapeLabel', 'closed', 'extra', 'start', 'foobar']
    config_dict["extra_shape_elements"] = ["foobar"]
    assert not Path(DEFAULT_CONFIGFILE).is_file()
    assert config_dict["shape_elements"] == ['shapeID', 'shapeLabel', 'closed', 'extra', 'start']
    config_dict["shape_elements"].extend(config_dict["extra_shape_elements"])
    assert config_dict["shape_elements"] == expected_shems

def test_warn_if_specify_both_nondefault_yamlstring_and_yamlfile(tmp_path):
    """Exit with ConfigError if tapshex_config specifies both YAML string and YAML file."""
    os.chdir(tmp_path)
...
def test_warn_if_specified_configfile_not_found(tmp_path):
    ...
    os.chdir(tmp_path)
    with pytest.raises(ConfigError) as err:
        tapshex_config(nondefault_configfile_name="passed_in_filename.yaml")
...
def test_warn_about_bad_yaml_if_yaml_parsererror(tmp_path, capsys):
    ...
    assert Path(DEFAULT_CONFIGFILE).is_file()
    assert "default_shape_identifier" in Path(DEFAULT_CONFIGFILE).read_text(encoding="utf-8")
    config_dict = tapshex_config()
...
def test_warn_of_bad_yaml_if_configfile_empty(tmp_path, capsys):
    ...
    config_dict = tapshex_config()
    assert Path(DEFAULT_CONFIGFILE).is_file()
    assert Path(DEFAULT_CONFIGFILE).read_text(encoding="utf-8") == "###"
    assert config_dict
    assert config_dict.get("default_shape_identifier")
    assert config_dict.get("csv_elements")  # computed
    assert config_dict.get("shape_elements")  # computed
...
def test_warn_if_yamlfile_found_with_bad_yaml(tmp_path, capsys):
    ...
    Path(DEFAULT_CONFIGFILE).write_text(bad_configyaml)
    x = tapshex_config()
    assert capsys.readouterr().err == "YAML is badly formed.\n"
...
def test_warn_if_yamlstring_found_with_bad_yaml(capsys):
    ...
    bad_configyaml = "DELIBE\nRATELY BAD: -: ^^YAML CONTENT^^\n"
    tapshex_config(nondefault_configyaml_str=bad_configyaml)
    assert capsys.readouterr().err == "YAML is badly formed.\n"
